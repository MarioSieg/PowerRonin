...
namespace dce::renderer::shaders {
	class BumpedDiffuse final : public IShader {
	public:
		explicit BumpedDiffuse(GPU& _gpu) noexcept;

		void load() override;
		void unload() override;

		void per_frame(const Vector4<>& _sun_dir, const Color<>& _sun_color, const Color<>& _ambient_color) const;
		void per_object(const Mesh& _mesh, const Material::BumpedDiffuse& _mat) const;
