...
namespace dce::renderer {
	...
	void render_stats(const Runtime& _runtime) {
		...
		bgfx::dbgTextPrintf(pos_x, ++pos_y, 0x8F, "OcclusionQueries: %u/%u", stats->numOcclusionQueries, caps->limits.maxOcclusionQueries);
		bgfx::dbgTextPrintf(pos_x, ++pos_y, 0x8F, "Position: %.2f %.2f %.2f, Forward: %.2f %.2f %.2f, Left: %.2f %.2f %.2f, FOV: %.1f", pos.x, pos.y, pos.z, forward.x, forward.y, forward.z, left.x, left.y, left.z, fov);
