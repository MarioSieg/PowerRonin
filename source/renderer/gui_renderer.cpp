...
namespace dce::renderer {
	auto GuiRenderer::initialize(const std::uint8_t font_size) -> bool {
		...
		std::int32_t lheight = 0;
		{
			...
			if (io.Fonts->AddFontFromFileTTF("fonts/jet_brains_mono_regular.ttf", static_cast<float>(font_size) - 3.F, &config
			                                 , ranges) == nullptr) {
				return false;
			}
			...
			*/
			if (!io.Fonts->AddFontFromFileTTF("fonts/font_awesome_pro_regular.ttf", static_cast<float>(font_size) - 3.F, &config
			                                  , glyph_ranges)) {
				return false;
			}
