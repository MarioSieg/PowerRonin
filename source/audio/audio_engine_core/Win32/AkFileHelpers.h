...
public:
	...
	static auto CloseFile(AkFileHandle in_hFile) -> AKRESULT {
	...
	static auto ReadBlocking(
		...
		if ( SetFilePointerEx( in_hFile, uPosition, NULL, FILE_BEGIN ) == FALSE )
			return AK_Fail;
#else
		if (SetFilePointer(in_hFile, in_uPosition, nullptr, FILE_BEGIN) != in_uPosition)
			return AK_Fail;
#endif
		if (::ReadFile(in_hFile, in_pBuffer, in_uSizeToRead, &out_uSizeRead, nullptr))
			return AK_Success;
		return AK_Fail;
	}
	...
	static auto CheckDirectoryExists(const AkOSChar* in_pszBasePath) -> AKRESULT {
	...
	static auto WriteBlocking(
		...
		AkUInt32 in_uSizeToWrite) -> AKRESULT {
