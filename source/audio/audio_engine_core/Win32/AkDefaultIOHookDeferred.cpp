...
auto CAkDefaultIOHookDeferred::Init(
...
auto CAkDefaultIOHookDeferred::Open(
...
VOID CALLBACK CAkDefaultIOHookDeferred::FileIOCompletionRoutine(
  ...
  dwNumberOfBytesTransfered
	...
	auto* pXferInfo = static_cast<AkAsyncIOTransferInfo*>(lpOverlapped->hEvent);
	...
	if (ERROR_SUCCESS == dwErrorCode) {
		eResult = AK_Success;
		AKASSERT(
			dwNumberOfBytesTransfered >= pXferInfo->uRequestedSize && dwNumberOfBytesTransfered <= pXferInfo->
			uBufferSize);
	}
...
auto CAkDefaultIOHookDeferred::Read(
...
auto CAkDefaultIOHookDeferred::Write(
	AkFileDesc& in_fileDesc, // File descriptor.
	const AkIoHeuristics& /*in_heuristics*/, // Heuristics for this data transfer (not used in this implementation).
...
// Close a file.
auto CAkDefaultIOHookDeferred::Close(
	AkFileDesc& in_fileDesc // File descriptor.
) -> AKRESULT {
...
auto CAkDefaultIOHookDeferred::GetBlockSize(
	AkFileDesc& in_fileDesc // File descriptor.
) -> AkUInt32 {
	if (in_fileDesc.pCustomParam == nullptr)
...
auto CAkDefaultIOHookDeferred::GetDeviceData() -> AkUInt32 {
