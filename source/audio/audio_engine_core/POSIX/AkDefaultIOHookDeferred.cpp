...
void AioFuncRead(AkAsyncIOTransferInfo& transferInfo) {
	...
	if (lseek(fileno(fileHandle), transferInfo.uFilePosition, SEEK_SET) == transferInfo.uFilePosition) {
		// Read the data
		AkUInt32 byteTransferred = static_cast<AkUInt32>(read(fileno(fileHandle), transferInfo.pBuffer,
...
void AioFuncWrite(AkAsyncIOTransferInfo& transferInfo) {
	...
	if (lseek(fileno(fileHandle), transferInfo.uFilePosition, SEEK_SET) == transferInfo.uFilePosition) {
		// Read the data
		AkUInt32 byteTransferred = static_cast<AkUInt32>(write(fileno(fileHandle), transferInfo.pBuffer,
		                                                       transferInfo.uRequestedSize));
