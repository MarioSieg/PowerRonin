...
public:
	...
	static AkForceInline auto Get(T* in_pItem) -> T*& {
...
public:
	...
	static auto Create(
		...
		AkUInt8* pToRelease = static_cast<AkUInt8*>(AkMalign(AkMemID_FilePackage, uMemSize, in_uBlockAlign));
		if (!pToRelease)
			return nullptr;
		...
		T_PACKAGE* pFilePackage = AkPlacementNew(pToRelease + out_uReservedHeaderSize) T_PACKAGE(
			uPackageID, in_uHeaderSize, pToRelease);
	...
	// Getters.
	[[nodiscard]] inline auto ID() const -> AkUInt32 { return m_uPackageID; }
	inline auto HeaderSize() const -> AkUInt32 { return m_uHeaderSize; }
...
protected:
	...
	virtual ~CAkFilePackage() = default;
...
//-----------------------------------------------------------------------------
using ListFilePackages = AkListBare<CAkFilePackage, CAkListAware, AkCountPolicyWithCount>;
