...
template <class TYPE>
inline
auto AkTemplMax(const TYPE& in_left, const TYPE& in_right) -> const TYPE& {
...
auto CAkFileLocationBase::GetFullFilePath(
...
auto CAkFileLocationBase::GetFullFilePath(
	...
	if (!in_pFlags ||
		!(in_pFlags->uCompanyID == AKCOMPANYID_AUDIOKINETIC || in_pFlags->uCompanyID ==
			AKCOMPANYID_AUDIOKINETIC_EXTERNAL)) {
...
auto CAkFileLocationBase::SetBasePath(
	const AkOSChar* in_pszBasePath
) -> AKRESULT {
	size_t len = AKPLATFORM::OsStrLen(in_pszBasePath) + 2; //+2 for possible missing slash
	if (len + AkTemplMax(AKPLATFORM::OsStrLen(m_szBankPath), AKPLATFORM::OsStrLen(m_szAudioSrcPath)) +
		AKPLATFORM::OsStrLen(AK::StreamMgr::GetCurrentLanguage()) + 1 >= AK_MAX_PATH) {
		return AK_InvalidParameter;
	}
	...
	if (len > 2)
		// Ensure path has terminating separator, and validate existence of directory, except for an empty directory (==current directory)
	{
...
auto CAkFileLocationBase::SetBankPath(
	const AkOSChar* in_pszBankPath
) -> AKRESULT {
	if (AKPLATFORM::OsStrLen(m_szBasePath) + AkTemplMax(AKPLATFORM::OsStrLen(in_pszBankPath),
	                                                    AKPLATFORM::OsStrLen(m_szAudioSrcPath)) + AKPLATFORM::OsStrLen(
		AK::StreamMgr::GetCurrentLanguage()) + 1 >= AK_MAX_PATH) {
		return AK_InvalidParameter;
	}
...
auto CAkFileLocationBase::SetAudioSrcPath(
	const AkOSChar* in_pszAudioSrcPath
) -> AKRESULT {
	if (AKPLATFORM::OsStrLen(m_szBasePath) + AkTemplMax(AKPLATFORM::OsStrLen(m_szBankPath),
	                                                    AKPLATFORM::OsStrLen(in_pszAudioSrcPath)) +
		AKPLATFORM::OsStrLen(AK::StreamMgr::GetCurrentLanguage()) + 1 >= AK_MAX_PATH) {
		return AK_InvalidParameter;
	}
