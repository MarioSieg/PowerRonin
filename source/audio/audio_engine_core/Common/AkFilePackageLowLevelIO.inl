...
AKRESULT CAkFilePackageLowLevelIO<T_LLIOHOOK_FILELOC, T_PACKAGE>::_LoadFilePackage(
	...
	out_pPackage = T_PACKAGE::Create(
		...
		uFileHeader.uHeaderSize + AKPK_HEADER_CHUNK_DEF_SIZE,
	...
	if (uSizeRead > sizeof(AkFilePackageHeader)) {
		...
		uHeaderSize = (uHeaderSize + in_reader.GetBlockSize() - 1) / in_reader.GetBlockSize() * in_reader.
			GetBlockSize();
	...
	if (uHeaderSize > 0) {
		...
		if (in_reader.Read(pFilePackageHeader + uHeaderReadOffset, uHeaderSize, uSizeRead, in_readerPriority) !=
			AK_Success
