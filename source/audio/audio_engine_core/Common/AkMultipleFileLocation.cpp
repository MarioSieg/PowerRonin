...
auto CAkMultipleFileLocation::Open(
	...
	if (m_Locations.IsEmpty()) {
		...
		if (GetFullFilePath(in_pszFileName, in_pFlags, in_eOpenMode, szFullFilePath) == AK_Success) {
			...
			if (res == AK_Success) {
				...
				AKASSERT(
					out_fileDesc.iFileSize != 0 && (in_eOpenMode == AK_OpenModeRead || in_eOpenMode ==
						AK_OpenModeReadWrite) || !(in_eOpenMode == AK_OpenModeRead || in_eOpenMode ==
...
auto CAkMultipleFileLocation::GetFullFilePath(
	const AkOSChar* in_pszFileName, // File name.
...
auto CAkMultipleFileLocation::AddBasePath(const AkOSChar* in_pszBasePath) -> AKRESULT {
	auto origLen = static_cast<AkUInt32>(AKPLATFORM::OsStrLen(in_pszBasePath));
	...
	auto* pPath = static_cast<FilePath*>(AkAlloc(AkMemID_Streaming,
	                                             sizeof(FilePath) + sizeof(AkOSChar)*(newByteSize - 1)));
	if (pPath == nullptr)
