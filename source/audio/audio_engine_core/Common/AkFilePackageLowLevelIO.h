...
public:
	...
	AKRESULT Open(
		...
		bool in_bReadFromSFXOnlyDir
		// If true, the file package is opened from the language agnostic directory only. Otherwise, it tries to open it 
	...
	[[nodiscard]] AkUInt64 GetSize() const {
		AKASSERT(m_pStream);
		AkStreamInfo info;
	...
	[[nodiscard]] AkFileHandle GetHandle() const {
		AKASSERT(m_pStream);
...
public:
	...
	[[nodiscard]] inline AkFileHandle GetHandleForFileDesc() const { return m_hFile; }
	inline auto GetCustomParamPtrForFileDesc() const -> void* { return m_pCustomParam; }

	AkFileDesc* GetFileDesc() { return m_reader.GetFileDesc(); }

protected:

	AkFilePackageReader m_reader;
	// READER object. Holds the stream used to read the package. Closed only upon package destruction.
...
protected:
	...
	static AK_FUNC(void, LanguageChangeHandler)(
		...
		static_cast<CAkFilePackageLowLevelIO<T_LLIOHOOK_FILELOC, T_PACKAGE>*>(in_pCookie)->OnLanguageChange(
			in_pLanguageName);
	}
