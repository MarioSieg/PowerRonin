...
CAkDefaultIOHookBlocking::CAkDefaultIOHookBlocking(): m_deviceID(AK_INVALID_DEVICE_ID), m_bAsyncOpen(false) {}
...
AKRESULT CAkDefaultIOHookBlocking::Init(const AkDeviceSettings& in_deviceSettings,
                                        // Device settings.
	...
	if (m_deviceID != AK_INVALID_DEVICE_ID) return AK_Success;
...
AKRESULT CAkDefaultIOHookBlocking::Open(const AkOSChar* in_pszFileName,
                                        // File name.
	...
	if (io_bSyncOpen || !m_bAsyncOpen) {
		io_bSyncOpen = true;
		const AKRESULT eResult = CAkMultipleFileLocation::Open(in_pszFileName, in_eOpenMode, in_pFlags, false, out_fileDesc);
...
AKRESULT CAkDefaultIOHookBlocking::Open(AkFileID in_fileID,
                                        // File ID.
                                        AkOpenMode in_eOpenMode,
                                        // Open mode.
                                        AkFileSystemFlags* in_pFlags,
	...
	if (io_bSyncOpen || !m_bAsyncOpen) {
		io_bSyncOpen = true;
		const AKRESULT eResult = CAkMultipleFileLocation::Open(in_fileID, in_eOpenMode, in_pFlags, false, out_fileDesc);
		if (eResult == AK_Success) {
			out_fileDesc.uSector = 0;
			out_fileDesc.deviceID = m_deviceID;
			out_fileDesc.pCustomParam = nullptr;
...
AKRESULT CAkDefaultIOHookBlocking::Read(AkFileDesc& in_fileDesc,
                                        // File descriptor.
	...
	overlapped.Offset = static_cast<DWORD>(io_transferInfo.uFilePosition & 0xFFFFFFFF);
	overlapped.OffsetHigh = static_cast<DWORD>(io_transferInfo.uFilePosition >> 32 & 0xFFFFFFFF);
	overlapped.hEvent = nullptr;

	AkUInt32 uSizeTransferred;

	if (::ReadFile(in_fileDesc.hFile, out_pBuffer, io_transferInfo.uRequestedSize, &uSizeTransferred, &overlapped)) {
...
AKRESULT CAkDefaultIOHookBlocking::Write(AkFileDesc& in_fileDesc,
                                         // File descriptor.
                                         const AkIoHeuristics& /*in_heuristics*/,
                                         // Heuristics for this data transfer (not used in this implementation).
                                         void* in_pData,
	...
	AKASSERT(in_pData && in_fileDesc.hFile != INVALID_HANDLE_VALUE);
	...
	if (::WriteFile(in_fileDesc.hFile, in_pData, io_transferInfo.uRequestedSize, &uSizeTransferred, &overlapped)) {
		AKASSERT(uSizeTransferred == io_transferInfo.uRequestedSize);
		return AK_Success;
	}
	return AK_Fail;
...
// Cleans up a file.
AKRESULT CAkDefaultIOHookBlocking::Close(AkFileDesc& in_fileDesc // File descriptor.
) {
	return CAkFileHelpers::CloseFile(in_fileDesc.hFile);
}

// Returns the block size for the file or its storage device. 
AkUInt32 CAkDefaultIOHookBlocking::GetBlockSize(AkFileDesc& /*in_fileDesc*/ // File descriptor.
) {
...
void CAkDefaultIOHookBlocking::GetDeviceDesc(AkDeviceDesc&
#ifndef AK_OPTIMIZED
