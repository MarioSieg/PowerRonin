...
CAkDefaultIOHookDeferred::CAkDefaultIOHookDeferred(): m_deviceID(AK_INVALID_DEVICE_ID), m_bAsyncOpen(false) {}
...
AKRESULT CAkDefaultIOHookDeferred::Init(const AkDeviceSettings& in_deviceSettings,
                                        // Device settings.
	...
	if (!AK::StreamMgr::GetFileLocationResolver()) AK::StreamMgr::SetFileLocationResolver(this);
...
AKRESULT CAkDefaultIOHookDeferred::Open(const AkOSChar* in_pszFileName,
                                        // File name.
...
AKRESULT CAkDefaultIOHookDeferred::Open(AkFileID in_fileID,
                                        // File ID.
                                        AkOpenMode in_eOpenMode,
                                        // Open mode.
                                        AkFileSystemFlags* in_pFlags,
...
VOID CALLBACK CAkDefaultIOHookDeferred::FileIOCompletionRoutine(DWORD dwErrorCode,
                                                                DWORD
#ifdef _DEBUG
...
AKRESULT CAkDefaultIOHookDeferred::Read(AkFileDesc& in_fileDesc,
                                        // File descriptor.
	...
	AKASSERT(in_fileDesc.hFile != INVALID_HANDLE_VALUE && io_transferInfo.uRequestedSize > 0 && io_transferInfo.uBufferSize >= io_transferInfo.uRequestedSize);
	...
	AKASSERT(( io_transferInfo.uFilePosition % WIN32_NO_BUFFERING_BLOCK_SIZE ) == 0 || !"Requested file position for I/O transfer is inconsistent with block size");
	...
	if (::ReadFileEx(in_fileDesc.hFile, io_transferInfo.pBuffer, io_transferInfo.uBufferSize, pOverlapped, CAkDefaultIOHookDeferred::FileIOCompletionRoutine)) {
		return AK_Success;
	}
...
AKRESULT CAkDefaultIOHookDeferred::Write(AkFileDesc& in_fileDesc,
                                         // File descriptor.
                                         const AkIoHeuristics& /*in_heuristics*/,
                                         // Heuristics for this data transfer (not used in this implementation).
                                         AkAsyncIOTransferInfo& io_transferInfo // Platform-specific asynchronous IO operation info.
) {
	AKASSERT(in_fileDesc.hFile != INVALID_HANDLE_VALUE && io_transferInfo.uRequestedSize > 0);
	...
	AKASSERT(io_transferInfo.uFilePosition % GetBlockSize( in_fileDesc ) == 0 || !"Requested file position for I/O transfer is inconsistent with block size");
	...
	// File was open with asynchronous flag. 
	// Read overlapped. 
	if (::WriteFileEx(in_fileDesc.hFile, io_transferInfo.pBuffer, io_transferInfo.uRequestedSize, pOverlapped, CAkDefaultIOHookDeferred::FileIOCompletionRoutine)) {
		return AK_Success;
	}
	ReleaseOverlapped(pOverlapped);
	return AK_Fail;
...
// Cancel transfer(s).
void CAkDefaultIOHookDeferred::Cancel(AkFileDesc& in_fileDesc,
                                      // File descriptor.
                                      AkAsyncIOTransferInfo& /*io_transferInfo*/,
                                      // Transfer info to cancel (this implementation only handles "cancel all").
...
// Close a file.
AKRESULT CAkDefaultIOHookDeferred::Close(AkFileDesc& in_fileDesc // File descriptor.
) {
...
void CAkDefaultIOHookDeferred::GetDeviceDesc(AkDeviceDesc&
#ifndef AK_OPTIMIZED
