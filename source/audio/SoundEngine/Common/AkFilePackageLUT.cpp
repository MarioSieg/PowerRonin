...
CAkFilePackageLUT::CAkFilePackageLUT(): m_curLangID(AK_INVALID_LANGUAGE_ID), m_pLangMap(NULL), m_pSoundBanks(NULL), m_pStmFiles(NULL), m_pExternals(NULL) {
...
AKRESULT CAkFilePackageLUT::Setup(AkUInt8* in_pData,
                                  // Header data.
	...
	// Check header size,
	if (in_uHeaderSize < sizeof(FileHeaderFormat) + pHeader->uLanguageMapSize + pHeader->uSoundBanksLUTSize + pHeader->uStmFilesLUTSize + pHeader->uExternalsLUTSize) {
		return AK_Fail;
	}

	// Check version.
	if (pHeader->uVersion < AKPK_CURRENT_VERSION) return AK_Fail;
...
const CAkFilePackageLUT::AkFileEntry<AkFileID>* CAkFilePackageLUT::LookupFile(AkFileID in_uID,
                                                                              // File ID.
	...
	if (in_pFlags->uCodecID == AKCODECID_BANK && m_pSoundBanks && m_pSoundBanks->HasFiles()) {
...
// Find a file entry by ID.
const CAkFilePackageLUT::AkFileEntry<AkUInt64>* CAkFilePackageLUT::LookupFile(AkUInt64 in_uID,
                                                                              // File ID.
                                                                              AkFileSystemFlags* in_pFlags // Special flags. Do not pass NULL.
) {
	AKASSERT(in_pFlags);

	if (in_pFlags->uCompanyID == AKCOMPANYID_AUDIOKINETIC_EXTERNAL && m_pExternals && m_pExternals->HasFiles()) {
...
AKRESULT CAkFilePackageLUT::SetCurLanguage(const AkOSChar* in_pszLanguage // Language string.
) {
	m_curLangID = AK_INVALID_LANGUAGE_ID;
	if (m_pLangMap && in_pszLanguage) {
		AkUInt16 uLangID = (AkUInt16)m_pLangMap->GetID(in_pszLanguage);
		if (uLangID == AK_INVALID_UNIQUE_ID && m_pLangMap->GetNumStrings() > 1) // Do not return AK_InvalidLanguage if package contains only SFX data.
		{
...
AkFileID CAkFilePackageLUT::GetSoundBankID(const AkOSChar* in_pszBankName // Soundbank name.
) {
...
AkUInt64 CAkFilePackageLUT::GetExternalID(const AkOSChar* in_pszExternalName // External Source name.
) {
	char* szString;
...
AkUInt32 CAkFilePackageLUT::StringMap::GetID(const AkOSChar* in_pszString) {
	...
	do {
		...
		if (0 == iCmp) return pTable[uThis].uID;
