...
public:
	...
	AKRESULT Open(const AkOSChar* in_pszFilePackageName,
	              // File package name. Location is resolved using base class' Open().
		...
		if (eResult != AK_Success && !in_bReadFromSFXOnlyDir) {
			// Try again, in SFX-only directory.
			flags.bIsLanguageSpecific = false;
			eResult = AK::IAkStreamMgr::Get()->CreateStd(in_pszFilePackageName, &flags, AK_OpenModeRead, m_pStream, true);
		}

		return eResult;
	}

	AKRESULT Read(void* in_pBuffer,
	              // Buffer. Must be aligned with value returned by GetBlockSize().
	              AkUInt32 in_uSizeToRead,
	              // Size to read. Must be a multiple of value returned by GetBlockSize().
	              AkUInt32& out_uSizeRead,
	              // Returned size read.
	...
	AkUInt32 GetBlockSize() {
		...
		if (!m_uBlockSize) m_uBlockSize = m_pStream->GetBlockSize();
		return m_uBlockSize;
	}
...
public:
	...
	static CAkDiskPackage* Create(AkFilePackageReader& in_reader,
	                              // File package reader.
	                              const AkOSChar* in_pszPackageName,
	                              // Name of the file package (for memory monitoring).
	                              AkUInt32 in_uHeaderSize,
	                              // File package header size, including the size of the header chunk AKPK_HEADER_CHUNK_DEF_SIZE.
	                              AkUInt32& out_uReservedHeaderSize,
	                              // Size reserved for header, taking mem align into account.
	                              AkUInt8*& out_pHeaderBuffer // Returned address of memory for header.
	) {
		CAkDiskPackage* pPackage = CAkFilePackage::Create<CAkDiskPackage>(in_pszPackageName, in_uHeaderSize, in_reader.GetBlockSize(), out_uReservedHeaderSize, out_pHeaderBuffer);
		if (pPackage) {
...
public:
	...
	virtual AKRESULT LoadFilePackage(const AkOSChar* in_pszFilePackageName,
	                                 // File package name. Location is resolved using base class' Open().
	...
	virtual AKRESULT UnloadFilePackage(AkUInt32 in_uPackageID // Returned package ID.
	);
...
protected:
	...
	static AK_FUNC(void, LanguageChangeHandler)(const AkOSChar* const in_pLanguageName,
	                                            // New language name.
	...
	void OnLanguageChange(const AkOSChar* const in_pLanguageName // New language name.
	);
	...
	AKRESULT _LoadFilePackage(const AkOSChar* in_pszFilePackageName,
	                          // File package name. Location is resolved using base class' Open().
	...
	AKRESULT FindPackagedFile(T_PACKAGE* in_pPackage,
	...
	inline bool IsInPackage(const AkFileDesc& in_fileDesc // File descriptor.
	) {
