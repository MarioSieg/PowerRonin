...
using PowerRonin.Json.Converters;
...
{
    /// <summary>
    ///     Used by <see cref="JsonSerializer" /> to resolve a <see cref="JsonContract" /> for a given
...
#pragma warning restore 618
        ...
        private static bool FilterMembers(MemberInfo member)
        {
            ...
            {
                if (ReflectionUtils.IsIndexedProperty(property)) return false;

                return !ReflectionUtils.IsByRefLikeType(property.PropertyType);
            }

            if (member is FieldInfo field) return !ReflectionUtils.IsByRefLikeType(field.FieldType);
...
#pragma warning restore 618
        ...
        protected virtual JsonObjectContract CreateObjectContract(Type objectType)
        {
            ...
            if (extensionDataMember != null) SetExtensionDataDelegates(contract, extensionDataMember);
        ...
        private MemberInfo GetExtensionDataMemberForType(Type type)
        {
            ...
            var extensionDataMember = members.LastOrDefault(m =>
            {
                ...
                if (memberType != MemberTypes.Property && memberType != MemberTypes.Field) return false;
        ...
        private static void SetExtensionDataDelegates(JsonObjectContract contract, MemberInfo member)
        {
            ...
            if (extensionDataAttribute == null) return;
            ...
            EnumerableDictionaryWrapper<TEnumeratorKey, TEnumeratorValue> : IEnumerable<KeyValuePair<object, object>>
        {
            ...
            {
                foreach (var item in _e) yield return new KeyValuePair<object, object>(item.Key!, item.Value!);
            }
        ...
        private ConstructorInfo? GetAttributeConstructor(Type objectType)
        {
            ...
            if (en.MoveNext())
            {
                ...
                if (en.MoveNext()) throw new JsonException("Multiple constructors with the JsonConstructorAttribute.");
 ...
 JsonTypeReflector.GetAttribute<JsonContainerAttribute>(objectType);
        ...
        protected virtual JsonContract CreateContract(Type objectType)
        {
            ...
            if (IsJsonPrimitiveType(t)) return CreatePrimitiveContract(objectType);
            ...
            if (typeof(IEnumerable).IsAssignableFrom(t)) return CreateArrayContract(objectType);
        ...
        {
            if (type.IsGenericTypeDefinition() || !type.ContainsGenericParameters()) return type.FullName;
...
#pragma warning restore 618
        ...
        {
            if (NamingStrategy != null) return NamingStrategy.GetExtensionDataName(extensionDataName);

            return extensionDataName;
        }

        /// <summary>
        ///     Resolves the key of the dictionary. By default <see cref="ResolvePropertyName" /> is used to resolve dictionary
        ///     keys.
        /// </summary>
