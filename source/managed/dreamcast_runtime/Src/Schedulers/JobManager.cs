...
namespace FluentScheduler
{
    ...
    public static class JobManager
    {
        ...
        {
            private get { return _jobFactory = _jobFactory ?? new JobFactory(); }
        ...
        private static void ScheduleJobs()
        {
            ...
            if (firstJob.NextRun <= Now)
            {
                ...
                if (firstJob.NextRun <= Now || firstJob.PendingRunOnce) _schedules.Remove(firstJob);
        ...
        internal static void RunJob(Schedule schedule)
        {
            ...
            var task = new Task(() =>
            {
                ...
                if (JobStart != null)
                    JobStart(
                        new JobStartInfo {Name = schedule.Name, StartTime = start}
                    );
                ...
                catch (Exception e)
                {
                    if (JobException != null)
                    {
                        ...
                        JobException(
                            new JobExceptionInfo {Name = schedule.Name, Exception = e}
                        );
                    }
                }
                finally
                {
                    ...
                    if (JobEnd != null)
                        JobEnd(
                            new JobEndInfo {Name = schedule.Name, StartTime = start, Duration = stopwatch.Elapsed, NextRun = schedule.NextRun}
                        );
                }
