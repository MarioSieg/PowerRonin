...
namespace dce::gui {
	constexpr auto imgui_rgba(const std::uint8_t r, const std::uint8_t g, const std::uint8_t b,
		...
		return static_cast<std::uint32_t>(r) << 0 | static_cast<std::uint32_t>(g) << 8 | static_cast<std::uint32_t>(b)
			<< 16 | static_cast<std::uint32_t>(a) << 24;
	}
	...
	inline void image(const bgfx::TextureHandle handle, const std::uint8_t flags, const std::uint8_t mip,
	...
	inline auto image_button(const bgfx::TextureHandle handle, const ImVec2& size,
	                         const ImVec2& uv0 = ImVec2(0.0F, 0.0F), const ImVec2& uv1 = ImVec2(1.0F, 1.0F),
	                         const int frame_padding = -1, const ImVec4& bg_col = ImVec4(0.0F, 0.0F, 0.0F, 0.0F),
	                         const ImVec4& tint_col = ImVec4(1.0F, 1.0F, 1.0F, 1.0F)) noexcept -> bool {
