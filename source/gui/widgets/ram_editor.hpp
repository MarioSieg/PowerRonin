...
struct MemoryEditor {
	...
	MemoryEditor() {
		...
		DataPreviewAddr = DataEditingAddr = static_cast<size_t>(-1);
		DataEditingTakeFocus = false;
		memset(DataInputBuf, 0, sizeof DataInputBuf);
		memset(AddrInputBuf, 0, sizeof AddrInputBuf);
		GotoAddr = static_cast<size_t>(-1);
	...
	void CalcSizes(Sizes& s, size_t mem_size, size_t base_display_addr) const {
		...
		s.SpacingBetweenMidCols = static_cast<float>(static_cast<int>(s.HexCellWidth * 0.25f));
	...
	void DrawContents(void* mem_data_void, size_t mem_size, size_t base_display_addr = 0x0000) {
		...
		ImU8* mem_data = static_cast<ImU8*>(mem_data_void);
		Sizes s;
		CalcSizes(s, mem_size, base_display_addr);
		ImGuiStyle& style = ImGui::GetStyle();
		...
		if (ReadOnly || DataEditingAddr >= mem_size) DataEditingAddr = static_cast<size_t>(-1);
		if (DataPreviewAddr >= mem_size) DataPreviewAddr = static_cast<size_t>(-1);
		...
		for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
		{
			...
			for (int n = 0; n < Cols && addr < mem_size; n++, addr++) {
				...
				if (DataEditingAddr == addr) {
					...
					if (ImGui::InputText("##data", DataInputBuf, 32, flags, UserData::Callback, &user_data)) data_write = data_next
						= true;
					else if (!DataEditingTakeFocus && !ImGui::IsItemActive()) DataEditingAddr = data_editing_addr_next = static_cast
						<size_t>(-1);
					DataEditingTakeFocus = false;
					ImGui::PopItemWidth();
					if (user_data.CursorPos >= 2) data_write = data_next = true;
					if (data_editing_addr_next != static_cast<size_t>(-1)) data_write = data_next = false;
					unsigned int data_input_value = 0;
					if (data_write && sscanf(DataInputBuf, "%X", &data_input_value) == 1) {
						if (WriteFn) WriteFn(mem_data, addr, static_cast<ImU8>(data_input_value));
						else mem_data[addr] = static_cast<ImU8>(data_input_value);
					}
					ImGui::PopID();
				}
				else {
					// NB: The trailing space is not visible but ensure there's no gap that the mouse cannot click on.
					const ImU8 b = ReadFn ? ReadFn(mem_data, addr) : mem_data[addr];

					if (OptShowHexII) {
						if (b >= 32 && b < 128) ImGui::Text(".%c ", b);
						else if (b == 0xFF && OptGreyOutZeroes) ImGui::TextDisabled("## ");
						else if (b == 0x00) ImGui::Text("   ");
						else ImGui::Text(format_byte_space, b);
					}
					else {
						if (b == 0 && OptGreyOutZeroes) ImGui::TextDisabled("00 ");
						else ImGui::Text(format_byte_space, b);
					}
			...
			if (OptShowAscii) {
				...
				for (int n = 0; n < Cols && addr < mem_size; n++, addr++) {
					...
					const unsigned char c = ReadFn ? ReadFn(mem_data, addr) : mem_data[addr];
		...
		else if (data_editing_addr_next != static_cast<size_t>(-1)) {
		...
		if (GotoAddr != static_cast<size_t>(-1)) {
			if (GotoAddr < mem_size) {
				...
				ImGui::SetScrollFromPosY(
					static_cast<float>(ImGui::GetCursorStartPos().y + GotoAddr / Cols * ImGui::GetTextLineHeight()));
				ImGui::EndChild();
	...
	void DrawPreviewLine(const Sizes& s, void* mem_data_void, size_t mem_size, size_t base_display_addr) {
		...
		char buf[128] = "";
		const float x = s.GlyphWidth * 6.0f;
		const bool has_value = DataPreviewAddr != static_cast<size_t>(-1);
		if (has_value)
			DrawPreviewData(DataPreviewAddr, mem_data, mem_size, PreviewDataType, DataFormat_Dec, buf
			                , static_cast<size_t>(IM_ARRAYSIZE(buf)));
		ImGui::Text("Dec");
		ImGui::SameLine(x);
	...
	void DrawPreviewData(size_t addr
		...
		uint8_t buf[8];
		const size_t elem_size = DataTypeGetSize(data_type);
		const size_t size = addr + elem_size > mem_size ? mem_size - addr : elem_size;
		if (ReadFn) for (int i = 0, n = static_cast<int>(size); i < n; ++i) buf[i] = ReadFn(mem_data, addr + i);
		else memcpy(buf, mem_data + addr, size);

		if (data_format == DataFormat_Bin) {
			uint8_t binbuf[8];
			EndianessCopy(binbuf, buf, size);
			ImSnprintf(out_buf, out_buf_size, "%s", FormatBinary(binbuf, static_cast<int>(size) * 8));
			return;
		}
		...
		case ImGuiDataType_S64: {
			...
			if (data_format == DataFormat_Dec) {
				ImSnprintf(out_buf, out_buf_size, "%lld", static_cast<long long>(int64));
				return;
			}
			if (data_format == DataFormat_Hex) {
				ImSnprintf(out_buf, out_buf_size, "0x%016llx", static_cast<long long>(int64));
				return;
			}
			break;
		}
		case ImGuiDataType_U64: {
			...
			if (data_format == DataFormat_Dec) {
				ImSnprintf(out_buf, out_buf_size, "%llu", static_cast<long long>(uint64));
				return;
			}
			if (data_format == DataFormat_Hex) {
				ImSnprintf(out_buf, out_buf_size, "0x%016llx", static_cast<long long>(uint64));
				return;
			}
			break;
		}
